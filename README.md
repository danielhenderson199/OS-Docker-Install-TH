...
```js
sudo su sudo apt update sudo apt install docker.io docker-compose mkdir cloudpc && cd cloudpc wget -O windows.yml https://raw.githubusercontent.com/MCFirsting/Windows-Docker-Install-TH/main/windows.yml sudo docker-compose -f windows.yml up
```
...
```js
sudo su cd cloudpc sudo docker-compose -f windows.yml start
```
